...
public abstract class DownloadSingerArtistMusic extends DownloadMusic {
    private SingerArtistMusic mSingerMusic;

    public DownloadSingerArtistMusic(Activity activity, SingerArtistMusic music){
        super(activity);
        mSingerMusic = music;
    }
    ...
    protected void download() {
        final String title = mSingerMusic.getTitle();
        final String artist = mSingerMusic.getAuthor();
        //下载歌词
        String lrcFileName = FileUtils.getLrcFileName(artist, title);
        File lrcFile = new File(FileUtils.getLrcDir()+lrcFileName);
        if (!lrcFile.exists()){
            downloadLrc(mSingerMusic.getSong_id(), lrcFileName);
        }
        ...
        //获取歌曲链接
        HttpClient.getMusicDownloadInfo(mSingerMusic.getSong_id(), new HttpCallback<DownloadInfo>() {
            @Override
